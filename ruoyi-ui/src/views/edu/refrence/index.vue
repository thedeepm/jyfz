    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['edu:refrence:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['edu:refrence:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listRefrence(this.queryParams).then((response) => {
        this.refrenceList = response.rows;
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.id != null) {
            updateRefrence(this.form).then((response) => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addRefrence(this.form).then((response) => {
    ...
    handleExport() {
      ...
      this.$confirm("是否确认导出所有流程图数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return exportRefrence(queryParams);
        })
        .then((response) => {
